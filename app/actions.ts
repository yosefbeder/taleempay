...
export async function searchStudents(query: string, classId?: number) {
  ...
  try {
    // Normalize query: replace all forms of alif with bare alif
    const normalizedQuery = query.replace(/[أإآ]/g, "ا");
    const searchPattern = `%${normalizedQuery}%`;
    const cid = classId ?? null;

    // Use raw query to handle normalization in the database
    // We normalize both name and settingId in the database for comparison
    const students = (await prisma.$queryRaw`
      SELECT * FROM "Student"
      WHERE (
        REPLACE(REPLACE(REPLACE("name", 'أ', 'ا'), 'إ', 'ا'), 'آ', 'ا') LIKE ${searchPattern}
        OR
        REPLACE(REPLACE(REPLACE("settingId", 'أ', 'ا'), 'إ', 'ا'), 'آ', 'ا') LIKE ${searchPattern}
      )
      AND (${cid}::integer IS NULL OR "classId" = ${cid}::integer)
      LIMIT 10
    `) as any[];

    // Map the raw results to ensure they match the Student type (specifically ensures types are correct if needed)
    return students;
  } catch (error) {
...
export async function getStudentOrder(studentId: string, productId: string) {
  try {
    ...
    if (
      ...
      !order.paymentScreenshotPath.startsWith("/")
    ) {
      try {
        ...
        return {
          ...order,
          paymentScreenshotPath: signedUrl,
          paymentScreenshotKey: order.paymentScreenshotPath,
        };
      } catch (e) {
...
export async function loginAdmin(username: string, password: string) {
  // Changed signature to match usage
  try {
...
export async function createOrder(formData: FormData) {
  ...
  const activationPhoneNumber = formData.get("activationPhoneNumber") as
    | string
    | null;
  ...
  try {
    ...
    const isHeic =
      screenshot.name.toLowerCase().endsWith(".heic") ||
  ...
  try {
    ...
    if (!student) {
      return {
        success: false,
        error: "الطالب غير موجود. الرجاء إعادة اختيار الطالب.",
      };
    }
    ...
    if (existingOrder) {
      order = await prisma.order.update({
        ...
        data: {
          ...
          activationPhoneNumber:
            activationPhoneNumber || existingOrder.activationPhoneNumber,
    ...
    return {
      success: true,
      order: { ...order, paymentScreenshotPath: signedUrl },
    };
  } catch (error: any) {
...
export async function getProductStats(productId: string) {
  try {
    ...
    const paidOrders = product.orders.filter(
      (o) => o.status === "PAID" || o.status === "DELIVERED",
    );
    const deliveredOrders = product.orders.filter(
      (o) => o.status === "DELIVERED",
    );
    const pendingPickup = product.orders.filter((o) => o.status === "PAID");
    const pendingConfirmation = product.orders.filter(
      (o) => o.status === "PENDING_CONFIRMATION",
    );
    const totalSales = paidOrders.length;

    const paidStudentIds = new Set(paidOrders.map((o) => o.studentId));
    const pendingConfirmationStudentIds = new Set(
      pendingConfirmation.map((o) => o.studentId),
    );
    ...
    const unpaidStudentsList = allStudents.filter(
      (s) =>
        !paidStudentIds.has(s.id) && !pendingConfirmationStudentIds.has(s.id),
    );

    // Generate Signed URLs for pending confirmation orders
    const pendingConfirmationOrdersWithUrls = await Promise.all(
      pendingConfirmation.map(async (o) => {
        let signedUrl = o.paymentScreenshotPath;

        // If path is a key (doesn't start with http or /), generate signed URL
        if (
          o.paymentScreenshotPath &&
          !o.paymentScreenshotPath.startsWith("http") &&
          !o.paymentScreenshotPath.startsWith("/")
        ) {
          try {
            const command = new GetObjectCommand({
              Bucket: process.env.R2_BUCKET_NAME!,
    ...
    return {
      ...
      stats: {
        ...
        unpaidStudentsList: unpaidStudentsList.map((s) => {
          ...
          const status =
            studentOrder?.status === "DECLINED" ? "DECLINED" : "UNPAID";
  ...
  try {
    ...
    if (order.status !== "PAID") {
      if (order.status === "PENDING_CONFIRMATION")
        throw new Error("الطلب في انتظار التأكيد");
